...
unsigned int bcs, dco, bcs_nonres = 0x06, dco_sweep, dco_max=0x60, bcs_sweep, bcs_max= 0x07;   // Initalizing Clock Parameters
...
{
  delay_adc = 100;                                // Delay Initialization
  ...
  {    
    ...
    {      
      ...
      for(d1=0; d1<delay_adc*30; d1++);              // Delay to stabilize voltage
...
int main (void)
{
  ...
  P1DIR |= 0x0F;  P1OUT = 0x03;             // Test pins
  
  P1DIR |= 0x10;  P1SEL |= 0x10;            // SET THE PIN DIRECTION FOR SMCLK OUT (P1.4)            

/* //Uncomment this portion of the code when you want Auto-resonance to be performed
...
/* //Uncomment this portion of the code when Calibration of Internal Threshold is required
  ...
  */
  ...
  // Code Segment for bypassing the Auto-resoance portion and setting up a fixed frequency
  
  bcs = bcs_max; dco = dco_max;                   // Setting BCS, DCO and BCS_NonRes(Half of Resonant Frequency) values 
  BCSCTL1 = bcs; DCOCTL = dco;                   // Setting SMCLK at Resonant Frequency
  
  freq  =  lut[bcs_max-3][(dco_max/32)-1];      // Estimates Delays for byte-transfer with the found Maximum BCS and DCO values
  delay1 = ((long int) (2.291e6*140/freq))/5;
  if(bcs_max > 8)
  {
    bcs_nonres = (bcs_max - 2);                // Non-resonant frequency is half of resonant frequency
    delay2 = delay1*2;                         // Multiplied by 2
